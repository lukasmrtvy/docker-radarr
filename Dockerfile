...
RUN addgroup -S ${GROUP} -g ${GID} && adduser -D -S -u ${UID} ${USER} ${GROUP}  && \
  ...
  apk -U add --no-cache \
    ...
    mono@testing \
    curl && \
  mkdir -p /config/ /opt/radarr && curl -sSL https://github.com/Radarr/Radarr/releases/download/v${VERSION}/Radarr.develop.${VERSION}.linux.tar.gz | tar xz -C /opt/radarr --strip-components=1 && \
